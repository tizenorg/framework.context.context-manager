 ...
 */
typedef enum {
	GESTURE_DOUBLE_TAP = 1,	/**< The device is tapped twice */
	GESTURE_MOVE_TO_EAR,	/**< The device is moved near to an ear */
	GESTURE_NO_MOVE,		/**< The device is being stopped for a while */
	GESTURE_PICK_UP,		/**< The device is picked up */
	GESTURE_SHAKE,			/**< The device is quickly moved back and forth */
	GESTURE_SNAP,			/**< The device is moved along an axis and back */
	GESTURE_TILT,			/**< The device is tilted */
	GESTURE_TURN_FACE_DOWN,	/**< The device is flipped from face to back */
	GESTURE_WRIST_UP,		/**< The user moves his wrist up (only for watch-type devices) */
} gesture_type_e;
 ...
 * @details	Check if the given gesture type is supported in the current device.@n
 *			Note that this is a sync function.
 *			It may involve IPC, it thus may not be able to return the result instantly.
 *
 * @since_tizen	2.3
 *
